...
import type { ContactData, SocialLinkData } from '@/lib/content';
...
export default function Footer({ contact, socials }: FooterProps) {
  ...
  const office = lang === 'id' ? contact.officeAddressId : contact.officeAddressEn;
  const op = lang === 'id' ? contact.operationalAddressId : contact.operationalAddressEn;
  const hubungiLabel = t('nav.hubungi');
            ...
            <ul className="space-y-4 text-sm">
              <li className="flex items-start gap-2.5">
                ...
                <span className="text-slate-400">
                  <strong>Office:</strong> <span>{office}</span>
                </span>
              </li>
              <li className="flex items-start gap-2.5">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#63a9ff]" />
                <span className="text-slate-400">
                  <strong>Operational:</strong> <span>{op}</span>
                </span>
              </li>
              <li className="flex items-center gap-2.5">
                <Phone className="h-4 w-4 shrink-0 text-[#63a9ff]" />
                <a href={`tel:${contact.phone}`} className="text-slate-400 transition-colors hover:text-white">{contact.phoneDisplay}</a>
              </li>
              <li className="flex items-center gap-2.5">
                <Mail className="h-4 w-4 shrink-0 text-[#e83b42]" />
                <a href={`mailto:${contact.email}`} className="font-medium text-slate-400 underline transition-colors hover:text-white">
                  {contact.email}
                </a>
              </li>
            </ul>
                ...
                <div className="flex items-center gap-4">
                  {socials.map((item) => (
                    <a
                      ...
                      className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-white/5 text-slate-300 transition-all hover:scale-110 hover:bg-[#0256eb] hover:text-white"
                      title={item.platform}
                      aria-label={item.platform}
                    >
                      {platformIcons[item.platform] || <span className="text-xs font-bold">{item.platform}</span>}
                    </a>
                  ))}
                </div>
              </>
